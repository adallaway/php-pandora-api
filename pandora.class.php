...
class Pandora
{
  private $_username;
  private $_password;
  private $_partner_username;
  private $_partner_password;
  private $_device_model;
  private $_encryption_cipher;
  private $_decryption_cipher;
  private $_endpoint_version;
  private $_json_endpoint;
  private $_xml_endpoint;
  private $_endpoint_mode;

  private $_current_params;
  private $_last_synctime;
  public $last_error;
  public $last_request_data;
  public $last_response_data;


  function __construct($partner_username = 'android', $endpoint_mode = 'json')
  {
    ...
    if(isset($available_partners[$partner_username])) $partner_username = 'android';

    $this->_partner_username = $partner_username;
    $this->_endpoint_mode = $endpoint_mode;
    ...
    if($endpoint_mode == 'xml')
    {
      $endpoint_host = 'www.pandora.com';
      $xml_endpoint_base = '/radio/xmlrpc/';
      $json_endpoint_base = '/radio/json/';
    }
    else
    {
      $endpoint_host = ($use_internal_tuner)? 'internal-tuner.pandora.com' : 'tuner.pandora.com';
      $xml_endpoint_base = '/services/xmlrpc/';
      $json_endpoint_base = '/services/json/';
    }

    $this->_json_endpoint = "//{$endpoint_host}{$json_endpoint_base}";
    $this->_xml_endpoint = "//{$endpoint_host}{$xml_endpoint_base}";

    $this->_endpoint_version = ($endpoint_mode == 'json') ? '5' : '35';
  }
  ...
  {
    if($this->_endpoint_mode == 'xml')
      return $this->sendXmlRequest($method, $data, $encrypted, $ssl);
    else
  ...
  protected function sendXmlRequest($method, $data, $encrypted = false, $ssl = false)
  {
    $protocol = (!$ssl) ? 'http' : 'https';

    $url_params = $this->_current_params + array('method' => $method);
    $url = ($protocol.':'.$this->_xml_endpoint.'v'.$this->_endpoint_version.'?'.http_build_query($url_params));

    $xmlrpc_request = xmlrpc_encode_request($method, $data);
    ...
    (
      ...
      'header' => "Content-Type: text/xml\r\n".
                  "Content-Length: ".strlen($xmlrpc_request)."\r\n",
      'content' => $xmlrpc_request
    )));

    $xml_response = file_get_contents($url, false, $context);
    if($xml_response === false)
    {
      $this->last_error = 'Failed to reach endpoint URL: '.$url;
      return false;
    }

    $this->last_response_data = htmlspecialchars($xml_response);

    $xmlrpc_response = xmlrpc_decode($xml_response);

    if(isset($xmlrpc_response['faultCode']))
    {
      $this->last_error = $xmlrpc_response['faultString'].' ('.$this->defineErrorCode($xmlrpc_response['faultCode']).')';
      return false;
    }
  ...
  private function defineErrorCode($error_code)
  {
    $error_codes = array
    (
      '0' => 'INTERNAL',
